        ...
        <div className="space-y-8 max-w-7xl mx-auto pb-12 flex flex-col h-full px-4 md:px-0">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                ...
                <div className="flex gap-3">
                    ...
                    <Button onClick={() => { resetForm(); setOpen(true) }} className="rounded-2xl px-8 h-12 font-bold bg-pure-black text-white dark:bg-white dark:text-pure-black shadow-xl hover:scale-[1.02] transition-all">
            ...
            <Dialog open={open} onOpenChange={setOpen}>
                <DialogContent className="sm:max-w-[480px] rounded-[3rem] border-none shadow-3xl p-0 overflow-hidden">
                    ...
                    <DialogFooter className="p-10 bg-black/5 dark:bg-white/5 flex gap-4">
                        <Button variant="ghost" onClick={() => setOpen(false)} className="rounded-2xl font-black px-8 h-12 uppercase tracking-widest text-[10px]">Abandonar</Button>
                        <Button onClick={handleSave} disabled={!title} className="rounded-2xl font-black bg-electric-blue text-white px-10 h-12 shadow-xl shadow-electric-blue/20 hover:scale-[1.05] transition-all uppercase tracking-widest text-[10px]">{editMode ? 'Salvar Alterações' : 'Iniciar Missão'}</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </div>
